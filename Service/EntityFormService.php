    ...
    public function validadeForm($form, $repository, $entity)
    {
            ...
            if (method_exists($entity, $getField) && !is_null($entity->$getField())) {
                ...
                if ($map['type'] == 'string') {
                    if (strlen($entity->$getField()) > $map['length']) {
                        $error = new FormError(
                            "Campo com valor maior que o permitido. Insira no máximo " . $map['length'] .
                            " caracteres."
                        );
                ...
                if ($map['type'] == 'decimal') {
                    ...
                    if (strlen($_float[0]) > $map['precision'] && $map['precision'] > 0) {
                        $error = new FormError(
                            "Valor maior que o permitido. Utilize somente " . $map['precision'] .
                            " casas numéricas à esquerda."
                        );
                        $form->get($field)->addError($error);
                        return false;
                    }
                ...
                if ($map['type'] == 'array') {
                    if ($form->get($field)->getConfig()->getType()->getName() == 'gallery') {
                        ...
                        if (array_key_exists($field, $old) &&
                                !is_null($old[$field]) &&
                                is_array($entity->$getField())) {
                            foreach ($old[$field] as $oldField) {
                                if ($oldField &&
                                        !in_array($oldField, $entity->$getField()) &&
                                        file_exists($oldField)) {
                                    unlink($oldField);
                                }
                            }
                        }
        ...
        foreach ($associationMappings as $association) {
            ...
            if ($association['type'] == ClassMetadataInfo::MANY_TO_ONE ||
                $association['type'] == ClassMetadataInfo::ONE_TO_ONE) {
            ...
            if ($association['type'] == ClassMetadataInfo::MANY_TO_MANY ||
                $association['type'] == ClassMetadataInfo::ONE_TO_MANY) {
    ...
    public function loadFieldsType($formType, &$listFields, $repository)
    {
        ...
        foreach ($listFields as $key => $field) {
            ...
            if (isset($associationMappings[$key])) {
                if ($associationMappings[$key]['type'] == ClassMetadataInfo::MANY_TO_MANY ||
                    $associationMappings[$key]['type'] == ClassMetadataInfo::ONE_TO_MANY) {
